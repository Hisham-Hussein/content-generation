...
**Clarity first, beauty second, variety third.** A visual must be immediately obvious to the deck's ICP — a non-specialist reads it in about three seconds, before any label. Beauty means visually rich and worth screenshotting. **Brief the idea, do not draw the slide:** generate-carousel reads the tenant brand kit and designs the artwork with free rein inside the brand, so a direction that dictates geometry becomes a ceiling on what it is allowed to make. Density is a virtue when the hierarchy is right, and a dull frame is under-briefed: fix it with a better idea, never by removing things. Variety is the last tie-breaker and never a licence to get obscure. **Literal first:** the default picture for a slide is a plain, direct drawing of that slide's own content — three layers as three stacked labelled bands, a failed run as numbered steps with one marked. An analogy asks the reader to translate between two worlds in two seconds using a mapping the caption never taught them, so it must be chosen only when it genuinely beats the literal drawing, and what it shows must already be named in the deck text. What the anti-jargon rule actually forbids is an object nobody can name on sight (a caliper, a core log, a title block), not a plain drawing of the subject itself. The full priority order, the literal-first rule, the object rule, the decodability gate and the beauty gate live in `references/visual-direction-craft.md` and are pasted verbatim into all four Step 5 agent prompts.
...
- **5a — Art-director (through-line + picture per slide, runs once).** One agent. Input: the whole deck's slide text, the theme, the ICP, the story arc. Output: (a) the **through-line** — what the deck argues beat by beat and the one thing each slide must land; and (b) a **picture per slide**, stated as the IDEA it must convey and what the hero is, never as a construction, plus the cover's single hero image, the emotional-staging plan, and the deck's mood (theme-matched, mood only, never subject matter). It does NOT pick a motif, a metaphor, a kit of shapes, or any drawing conventions — no stroke weights, fills or states. **Coherence comes from the brand kit** (typeface, weight scale, stroke weight, icon treatment, palette, tag pill, footer), which generate-carousel applies to every slide no matter what each visual draws — so buying coherence with repeated shapes is paying twice and getting a monotonous deck. Twenty slides should produce roughly twenty different pictures. It writes no per-slide `Visual:` lines.
...
- **5c — Visual critic (deck-level, runs once) + revise loop.** One agent reviews ALL directions together — the only vantage from which coherence defects are visible. It returns a punch-list in severity order: undecodable imagery, unearned analogies and unnameable objects first (all top severity), then beauty failures (uninspiring, or under-built from bare geometry where the brand kit's components were available), then convention drift, focal point, monotony (every pair, not just neighbours, plus the whole-deck read), color words, slide-placement words, real brands named, and Dim-13 label jargon. Route the findings back to the 5b author for a revise pass, then re-critique. Bound the loop to 2 rounds; if items remain after round 2, surface them to the user rather than looping forever. A per-slide critic is explicitly wrong here — coherence is a whole-deck property.
...
- **Drawing the slide instead of briefing it.** Stroke weights, fills, shape-by-shape construction, component names. The renderer treats the direction as a ceiling, so geometry in, wireframe out.
- **Briefing a picture whose meaning depends on material rendering.** Texture, grain, painterly light and shadow, hand-inked contours, still-life realism. The artwork is vector, so that comes out as noise. This is the only thing to steer around; everything else is open.
...
- **Answering a weak slide with subtraction.** Stripping a frame back to plain rectangles and calling the result restraint. A dull slide is under-built: it needs a better idea and the brand kit's real components, not fewer of them.
- **Reading a theme's mood word as an instruction to draw less.** "Calm", "editorial" and "restrained" describe palette, contrast and how loud the emphasis runs. They never license bare geometry, fewer components, or a thinner slide.
