...
- **Cover title is ONE line (mandatory).** Compose the cover headline so it renders on a single line at the kit's cover size. If it does not fit, cut words until it does; only if the wording cannot go lower, drop the font size (a smaller one-line hook beats a full-size three-line one). Two lines is the absolute ceiling and needs a deliberate reason; three is never acceptable. The Step 5 validator hard-fails any cover title that wraps.
...
- Cover title lines: the cover headline (`.c-cover-hook`) must render on exactly ONE line. Hard fail on any wrap. A hook broken across three ragged lines ("Agent / architecture has / a half-life") reads as broken typography, not as a scroll-stopper. Fix in this order: cut words first, then reduce the font size below the kit's cover size. Never let it wrap.
